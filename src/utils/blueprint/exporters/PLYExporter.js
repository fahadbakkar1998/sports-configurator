...
THREE.PLYExporter.prototype = {
	...
	parse: function ( object, onDone, options ) {
		...
		let excludeAttributes = options.excludeAttributes;
		let geomToBufferGeom = new WeakMap();
		...
		object.traverse( function ( child ) {
			...
			if ( child.isMesh === true ) {
				...
				if ( geometry.isGeometry === true ) {
					...
					let bufferGeometry = geomToBufferGeom.get( geometry ) || new THREE.BufferGeometry().setFromObject( mesh );
		...
		let includeIndices = excludeAttributes.indexOf( 'index' ) === - 1;
		...
		if ( options.binary === true ) {
			...
			let headerBin = new TextEncoder().encode( header );

			// 3 position values at 4 bytes
			// 3 normal values at 4 bytes
			// 3 color channels with 1 byte
			// 2 uv values at 4 bytes
			let vertexListLength = vertexCount * ( 4 * 3 + ( includeNormals ? 4 * 3 : 0 ) + ( includeColors ? 3 : 0 ) + ( includeUVs ? 4 * 2 : 0 ) );

			// 1 byte shape desciptor
			// 3 vertex indices at ${indexByteCount} bytes
			let faceListLength = includeIndices ? faceCount * ( indexByteCount * 3 + 1 ) : 0;
			let output = new DataView( new ArrayBuffer( headerBin.length + vertexListLength + faceListLength ) );
			...
			traverseMeshes( function ( mesh, geometry ) {
				...
				if ( includeIndices === true ) {
					...
					let faceIndexFunc = `setUint${indexByteCount * 8}`;
					if ( indices !== null ) {
