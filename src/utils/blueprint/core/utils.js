...
export class Utils {
  ...
  static pointDistanceFromLine(point, start, end) {
    let tPoint = Utils.closestPointOnLine(point, start, end);
    let tDx = point.x - tPoint.x;
    let tDy = point.y - tPoint.y;
  ...
  static closestPointOnLine(point, start, end) {
    ...
    let tA = point.x - start.x;
    let tB = point.y - start.y;
    let tC = end.x - start.x;
    let tD = end.y - start.y;
  ...
  static isClockwise(points) {
    ...
    let tNewPoints = Utils.map(points, function (p) {
      return {
        x: p.x - tSubX,
        y: p.y - tSubY,
      };
