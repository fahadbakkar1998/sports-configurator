...
export class Controller extends EventDispatcher {
  ...
  clickPressed(vec2) {
    ...
    let intersection = this.itemIntersection(this.mouse, this.selectedObject);
    if (intersection) {
  ...
  clickDragged(vec2) {
    let scope = this;
    this.mouse = vec2 || this.mouse;
    let intersection = scope.itemIntersection(this.mouse, this.selectedObject);
    if (intersection) {
      if (scope.isRotating()) {
  ...
  checkWallsAndFloors() {
    ...
    if (this.state == states.UNSELECTED && this.mouseoverObject == null) {
      // check walls
      let wallEdgePlanes = this.model.floorplan.wallEdgePlanes();
      let wallIntersects = this.getIntersections(
        this.mouse,
        wallEdgePlanes,
        true,
      );
      ...
      // check floors
      let floorPlanes = this.model.floorplan.floorPlanes();
      let floorIntersects = this.getIntersections(
        this.mouse,
        floorPlanes,
        false,
      );
  ...
  updateIntersections() {
    ...
    let hudObject = this.hud.getObject();
    if (hudObject != null) {
      let hudIntersects = this.getIntersections(
        this.mouse,
        hudObject,
        false,
        false,
    ...
    // check objects
    let items = this.model.scene.getItems();
    let intersects = this.getIntersections(this.mouse, items, false, true);
  ...
  //
  mouseToVec3(vec2) {
    let normVec2 = this.normalizeVector2(vec2);
    let vector = new Vector3(normVec2.x, normVec2.y, 0.5);
  ...
  freeMouse3D(vec2) {
    let distance;
    let pos = new Vector3();
    let vector = this.mouseToVec3(vec2);
