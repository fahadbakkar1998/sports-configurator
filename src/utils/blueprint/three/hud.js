...
//		this.three.needsUpdate();
	...
	rotateVector(item) 
	{
		let vec = new Vector3(0, 0,Math.max(item.halfSize.x, item.halfSize.z) + 1.4 + this.distance);
		return vec;
	}
	...
	makeObject(item) 
	{
		...
		let line = new LineSegments(this.makeLineGeometry(item),this.makeLineMaterial(this.rotating));
