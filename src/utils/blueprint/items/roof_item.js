...
export class RoofItem extends Item
{
	...
	{
			let g = roof.geometry;
			let result = {distance: Number.MAX_VALUE, contains: false, point: null, closestPoint: null};
			let closestPoint = null;
			for (let i=0;i< g.faces.length;i++)
			{
					let f = g.faces[i];
					let plane = new Plane();
					let triangle = new Triangle(g.vertices[f.a], g.vertices[f.b], g.vertices[f.c]);
